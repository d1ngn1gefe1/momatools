...
class AnnMerger:
    ...
    def _get_anns(self):
        for id_act in self.ann_phase1.id_act_to_ids_sact:
            ...
            ids_sact = [
                self.ann_phase1.get_id_sact(ann_sact) for ann_sact in anns_sact_phase1
            ]
            anns_sact_phase2 = [
                self.ann_phase2.anns_sact_raw[id_sact] for id_sact in ids_sact
            ]
    ...
    def _merge(self):
        ...
        for (
            ...
            sact = []
            for id_sact, ann_sact_phase1, ann_sact_phase2 in zip(
                ids_sact, anns_sact_phase1, anns_sact_phase2
            ):
                hoi = []
                ...
                for id_hoi, ann_hoi in zip(ids_hoi, ann_sact_phase2):
                    ...
                    anns_actor = ann_hoi["task_result"]["annotations"][0][
                        "slotsChildren"
                    ]
                            ...
                            actor.append(
                                {
                                    ...
                                    "bbox": [
                                        ann_actor.bbox.x,
                                        ann_actor.bbox.y,
                                        ann_actor.bbox.width,
                                        ann_actor.bbox.height,
                                    ],
                                }
                            )

                    anns_object = ann_hoi["task_result"]["annotations"][1][
                        "slotsChildren"
                    ]
                    anns_object = [
                        Entity(ann_object, cn2en) for ann_object in anns_object
                    ]
                    object = []
                            ...
                            object.append(
                                {
                                    ...
                                    "bbox": [
                                        ann_object.bbox.x,
                                        ann_object.bbox.y,
                                        ann_object.bbox.width,
                                        ann_object.bbox.height,
                                    ],
                                }
                            )

                    anns_binary = ann_hoi["task_result"]["annotations"][2][
                        "slotsChildren"
                    ]
                    anns_binary = [
                        Predicate(ann_binary, cn2en) for ann_binary in anns_binary
                    ]
                    rel, ta = [], []
                    for ann_binary in anns_binary:
                        for id_src, id_trg, cname in ann_binary.breakdown():
                            if not (
                                id_src in id_entity_remove
                                or id_trg in id_entity_remove
                                or cname == "REMOVE"
                            ):
                                if ann_binary.cname in [
                                    x[0] for x in self.ann_phase2.taxonomy_rel
                                ]:
                                    rel.append(
                                        {
                                            "class_name": cname,
                                ...
                                elif ann_binary.cname in [
                                    x[0] for x in self.ann_phase2.taxonomy_ta
                                ]:
                                    ta.append(
                                        {
                                            "class_name": cname,
                    ...
                    anns_unary = ann_hoi["task_result"]["annotations"][3][
                        "slotsChildren"
                    ]
                    anns_unary = [
                        Predicate(ann_unary, cn2en) for ann_unary in anns_unary
                    ]
                    att, ia = [], []
                    for ann_unary in anns_unary:
                        for id_src, cname in ann_unary.breakdown():
                            if not (id_src in id_entity_remove or cname == "REMOVE"):
                                if ann_unary.cname in [
                                    x[0] for x in self.ann_phase2.taxonomy_att
                                ]:
                                    att.append(
                                        {"class_name": cname, "source_id": id_src}
                                    )
                                elif ann_unary.cname in [
                                    x[0] for x in self.ann_phase2.taxonomy_ia
                                ]:
                                    ia.append(
                                        {"class_name": cname, "source_id": id_src}
                                    )
                    ...
                    hoi.append(
                        {
                            ...
                            "time": hms2s(ann_sact_phase1["start"])
                            + float(id_hoi_to_timestamp[id_hoi]),
                            "actors": actor,
                            "objects": object,
                            "attributes": att,
                            "relationships": rel,
            ...
            act = {
                "id": id_act,
                "class_name": cn2en[
                    ann_act["subactivity"][0]["orig_vid"].split("_")[0]
                ],
