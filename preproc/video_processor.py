...
class VideoProcessor:
    ...
    def trim_video(path_src, path_trg, start, end):
        ffmpeg.input(path_src).video.trim(start=start, end=end).setpts(
            "PTS-STARTPTS"
        ).output(path_trg).run()

    @staticmethod
    def sample_image(path_src, path_trg, time):
        ffmpeg.input(path_src, ss=time).output(path_trg, vframes=1).run()
    ...
    def resize_video(path_src, path_trg, size=320):
        ...
        if width < height:
            ffmpeg.input(path_src).video.filter("scale", size, -2).output(
                path_trg
            ).run()
        else:
            ffmpeg.input(path_src).video.filter("scale", -2, size).output(
                path_trg
            ).run()
    ...
    def trim_and_resize_video(path_src, path_trg, start, end, size=320):
        ...
        if width < height:
            ffmpeg.input(path_src).video.trim(start=start, end=end).setpts(
                "PTS-STARTPTS"
            ).filter("scale", size, -2).output(path_trg).run()
        else:
            ffmpeg.input(path_src).video.trim(start=start, end=end).setpts(
                "PTS-STARTPTS"
            ).filter("scale", -2, size).output(path_trg).run()
    ...
    def trim_act(self, resize=False, overwrite=False):
        ...
        for ann in self.anns:
            ...
            path_trg = os.path.join(
                self.dir_moma, "videos/activity", f"{ann_act['id']}.mp4"
            )
            assert os.path.exists(path_src)
            if not os.path.exists(path_trg) or overwrite:
    ...
    def trim_sact(self, resize=False, overwrite=False):
        ...
        for ann in self.anns:
            ...
            for ann_sact in anns_sact:
                path_src = os.path.join(self.dir_moma, "videos/raw", ann["file_name"])
                path_trg = os.path.join(
                    self.dir_moma, "videos/sub_activity", f"{ann_sact['id']}.mp4"
                )
                assert os.path.exists(path_src)
                if not os.path.exists(path_trg) or overwrite:
        ...
        paths_exist = glob.glob(
            os.path.join(self.dir_moma, "videos/sub_activity/*.mp4")
        )
        for path_exist in paths_exist:
            if path_exist not in paths_trg:
                os.remove(path_exist)
    ...
    def trim_hoi(self, duration=1, resize=False, overwrite=False):
        os.makedirs(
            os.path.join(self.dir_moma, "videos/interaction_video"), exist_ok=True
        )
            ...
            for ann_sact in anns_sact:
                ...
                for ann_hoi in anns_hoi:
                    path_src = os.path.join(
                        self.dir_moma, "videos/raw", ann["file_name"]
                    )
                    path_trg = os.path.join(
                        self.dir_moma,
                        "videos/interaction_video",
                        f"{ann_hoi['id']}.mp4",
                    )
                    assert os.path.exists(path_src)
                    if not os.path.exists(path_trg) or overwrite:
                        ...
                        assert math.isclose(
                            end - start, duration, rel_tol=1e-4
        ...
        paths_exist = glob.glob(
            os.path.join(self.dir_moma, "videos/interaction_video/*.mp4")
        )
        for path_exist in paths_exist:
            if path_exist not in paths_trg:
                os.remove(path_exist)
        ...
        for ann in self.anns:
            ...
            for ann_sact in anns_sact:
                ...
                for ann_hoi in anns_hoi:
                    ...
                    path_trg = os.path.join(
                        self.dir_moma, "videos/interaction", f"{ann_hoi['id']}.jpg"
                    )
                    assert os.path.exists(path_src)
                    if not os.path.exists(path_trg) or overwrite:
                        time = ann_hoi["time"]
