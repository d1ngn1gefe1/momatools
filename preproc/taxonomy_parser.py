...
class TaxonomyParser:
    def __init__(self, dir_moma):
        ...
        # LVIS mapper
        with open(osp.join(dir_moma, "anns_raw/taxonomy/lvis.csv")) as f:
            reader = csv.reader(f, delimiter=",")
    ...
    def dump(self, verbose=True):
        ...
        with open(osp.join(self.dir_moma, "anns/taxonomy/actor.json"), "w") as f:
            json.dump(self.taxonomy_actor, f, indent=2, sort_keys=True)
        with open(osp.join(self.dir_moma, "anns/taxonomy/object.json"), "w") as f:
            json.dump(self.taxonomy_object, f, indent=2, sort_keys=True)
        with open(
            osp.join(self.dir_moma, "anns/taxonomy/intransitive_action.json"), "w"
        ) as f:
            json.dump(self.taxonomy_ia, f, indent=2, sort_keys=True)
        with open(
            osp.join(self.dir_moma, "anns/taxonomy/transitive_action.json"), "w"
        ) as f:
            json.dump(self.taxonomy_ta, f, indent=2, sort_keys=True)
        with open(osp.join(self.dir_moma, "anns/taxonomy/attribute.json"), "w") as f:
            json.dump(self.taxonomy_att, f, indent=2, sort_keys=True)
        with open(osp.join(self.dir_moma, "anns/taxonomy/relationship.json"), "w") as f:
            json.dump(self.taxonomy_rel, f, indent=2, sort_keys=True)
        with open(osp.join(self.dir_moma, "anns/taxonomy/act_sact.json"), "w") as f:
            json.dump(self.taxonomy_act_sact, f, indent=2, sort_keys=True)
        with open(osp.join(self.dir_moma, "anns/taxonomy/cn2en.json"), "w") as f:
            json.dump(self.cn2en, f, ensure_ascii=False, indent=2, sort_keys=True)
        with open(osp.join(self.dir_moma, "anns/taxonomy/lvis.json"), "w") as f:
